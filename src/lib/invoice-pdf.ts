...
import { formatCurrency, type Currency } from './invoice'
...
function addContinuationHeader(page: PDFPage, invoice: Invoice, font: PDFFont, bold: PDFFont, y: number): number {
  const title = `${invoice.invoiceDetails.title} — continued`
  page.drawText(title, { x: 50, y, size: 13, font: bold, color: stringToRGB(invoice.accentColor) })
...
async function renderProfessionalPDF(pdfDoc: PDFDocument, invoice: Invoice, calc: ReturnType<typeof calculateInvoice>) {
  ...
  if (yPos < 180) { page = pdfDoc.addPage(pageSize); yPos = 760; yPos = addContinuationHeader(page, invoice, timesRoman, timesBold, yPos) }
  const summaryX = tableX + tableWidth - 200
  page.drawText(`Subtotal: ${formatPdfCurrency(calc.subtotal, invoice.invoiceDetails.currency)}`, { x: summaryX, y: yPos, size: 10, font: timesRoman })
  yPos -= 15
  ...
  page.drawLine({ start: { x: summaryX, y: yPos + 5 }, end: { x: summaryX + 200, y: yPos + 5 }, thickness: 1 })
  yPos -= 20
  page.drawText(`TOTAL: ${formatPdfCurrency(calc.grandTotal, invoice.invoiceDetails.currency)}`, { x: summaryX, y: yPos, size: 14, font: timesBold, color: accentRGB })
  page.drawText('Generated with CraftMyPage', { x: 50, y: 20, size: 8, font: timesRoman, color: rgb(0.6, 0.6, 0.6) })
}
...
async function renderMinimalPDF(pdfDoc: PDFDocument, invoice: Invoice, calc: ReturnType<typeof calculateInvoice>) {
  ...
  const drawHeader = () => {
    page.drawText(invoice.business.name, { x: 50, y: yPos, size: 24, font: helveticaBold })
    page.drawText(invoice.invoiceDetails.title, { x: 450, y: yPos, size: 20, font: helveticaBold })
    page.drawText(invoice.invoiceDetails.invoiceNumber, { x: 450, y: yPos - 25, size: 10, font: helvetica, color: rgb(0.5, 0.5, 0.5) })
    yPos -= 60
  }
  drawHeader()
  ...
  const col1 = 50
  const col2 = 350
  page.drawText('From', { x: col1, y: yPos, size: 10, font: helveticaBold }); page.drawText(invoice.business.name, { x: col1, y: yPos - 15, size: 10, font: helvetica }); if (invoice.business.email) page.drawText(invoice.business.email, { x: col1, y: yPos - 28, size: 9, font: helvetica, color: rgb(0.5, 0.5, 0.5) })
  page.drawText('To', { x: col2, y: yPos, size: 10, font: helveticaBold }); page.drawText(invoice.customer.name, { x: col2, y: yPos - 15, size: 10, font: helvetica })
  yPos -= 100
  page.drawText('Issued', { x: col1, y: yPos, size: 9, font: helvetica, color: rgb(0.5, 0.5, 0.5) }); page.drawText(invoice.invoiceDetails.issueDate, { x: col1, y: yPos - 12, size: 10, font: helveticaBold })
  page.drawText('Due', { x: col2, y: yPos, size: 9, font: helvetica, color: rgb(0.5, 0.5, 0.5) }); page.drawText(invoice.invoiceDetails.dueDate, { x: col2, y: yPos - 12, size: 10, font: helveticaBold })
  yPos -= 50

  invoice.lineItems.forEach((item) => {
    if (yPos < 90) { page = pdfDoc.addPage(pageSize); yPos = 760; yPos = addContinuationHeader(page, invoice, helvetica, helveticaBold, yPos) }
    const itemAmount = Math.max(0, item.quantity * item.unitPrice - Math.min(item.discount || 0, item.quantity * item.unitPrice))
    page.drawText((item.description || 'Item').substring(0, 70), { x: col1, y: yPos, size: 10, font: helveticaBold })
    page.drawText(formatPdfCurrency(itemAmount, invoice.invoiceDetails.currency), { x: 500, y: yPos, size: 10, font: helveticaBold })
    page.drawText(`${item.quantity} × ${formatPdfCurrency(item.unitPrice, invoice.invoiceDetails.currency)}`, { x: col1, y: yPos - 12, size: 9, font: helvetica, color: rgb(0.6, 0.6, 0.6) })
    yPos -= 30
  })

  if (yPos < 150) { page = pdfDoc.addPage(pageSize); yPos = 760; yPos = addContinuationHeader(page, invoice, helvetica, helveticaBold, yPos) }
  page.drawText(`Subtotal: ${formatPdfCurrency(calc.subtotal, invoice.invoiceDetails.currency)}`, { x: 400, y: yPos, size: 10, font: helvetica })
  yPos -= 15
  if (calc.discountAmount > 0) { page.drawText(`Discount: -${formatPdfCurrency(calc.discountAmount, invoice.invoiceDetails.currency)}`, { x: 400, y: yPos, size: 10, font: helvetica }); yPos -= 15 }
  if (calc.taxAmount > 0) { page.drawText(`Tax: ${formatPdfCurrency(calc.taxAmount, invoice.invoiceDetails.currency)}`, { x: 400, y: yPos, size: 10, font: helvetica }); yPos -= 15 }
  page.drawText(`Total: ${formatPdfCurrency(calc.grandTotal, invoice.invoiceDetails.currency)}`, { x: 400, y: yPos - 10, size: 14, font: helveticaBold })
...
async function renderModernPDF(pdfDoc: PDFDocument, invoice: Invoice, calc: ReturnType<typeof calculateInvoice>) {
  ...
  const drawHeader = () => {
    ...
    page.drawText(invoice.business.name, { x: 50, y: 770, size: 24, font: helveticaBold, color: rgb(1, 1, 1) })
    page.drawText(invoice.invoiceDetails.title, { x: 450, y: 770, size: 18, font: helveticaBold, color: rgb(1, 1, 1) })
    page.drawText(invoice.invoiceDetails.invoiceNumber, { x: 450, y: 750, size: 10, font: helvetica, color: rgb(1, 1, 1) })
  }
  drawHeader()
  let yPos = 680
  const col1 = 50
  const col2 = 350
  page.drawText('From', { x: col1, y: yPos, size: 10, font: helveticaBold }); page.drawText(invoice.business.name, { x: col1, y: yPos - 15, size: 9, font: helvetica }); if (invoice.business.email) page.drawText(invoice.business.email, { x: col1, y: yPos - 27, size: 9, font: helvetica })
  page.drawText('To', { x: col2, y: yPos, size: 10, font: helveticaBold }); page.drawText(invoice.customer.name, { x: col2, y: yPos - 15, size: 9, font: helvetica })
  yPos -= 80
  page.drawText('Issue Date', { x: col1, y: yPos, size: 8, font: helvetica, color: rgb(0.6, 0.6, 0.6) }); page.drawText(invoice.invoiceDetails.issueDate, { x: col1, y: yPos - 10, size: 9, font: helveticaBold })
  page.drawText('Due Date', { x: col2, y: yPos, size: 8, font: helvetica, color: rgb(0.6, 0.6, 0.6) }); page.drawText(invoice.invoiceDetails.dueDate, { x: col2, y: yPos - 10, size: 9, font: helveticaBold })
  yPos -= 50

  invoice.lineItems.forEach((item) => {
    if (yPos < 110) { page = pdfDoc.addPage(pageSize); yPos = 760; yPos = addContinuationHeader(page, invoice, helvetica, helveticaBold, yPos) }
    const itemAmount = Math.max(0, item.quantity * item.unitPrice - Math.min(item.discount || 0, item.quantity * item.unitPrice))
    page.drawText((item.description || 'Item').substring(0, 70), { x: col1, y: yPos, size: 10, font: helvetica })
    page.drawText(formatPdfCurrency(itemAmount, invoice.invoiceDetails.currency), { x: 500, y: yPos, size: 10, font: helveticaBold })
    yPos -= 22
  })

  if (yPos < 160) { page = pdfDoc.addPage(pageSize); yPos = 760; yPos = addContinuationHeader(page, invoice, helvetica, helveticaBold, yPos) }
  page.drawText(`Subtotal: ${formatPdfCurrency(calc.subtotal, invoice.invoiceDetails.currency)}`, { x: 360, y: yPos, size: 10, font: helvetica })
  yPos -= 15
