...
const tools = [
  {
    ...
    title: 'Invoice Maker',
    description: 'Create professional invoices with line items, taxes, discounts, GST details and downloadable PDFs in your browser.',
    icon: '₹',
    ...
    <div className="cmp-page">
      <Helmet>
        <script type="application/ld+json">{JSON.stringify({
          ...
          name: 'CraftMyPage',
          url: 'https://craftmypage.pages.dev/',
          applicationCategory: 'DesignApplication',
          operatingSystem: 'Web browser',
          description: 'Free browser-based invoice, invitation and resume creation tools with local drafts and PDF or image export.',
        ...
        <div className="cmp-container relative py-16 sm:py-20 lg:py-24">
          <div className="max-w-4xl">
            ...
            <div className="mt-8 flex flex-wrap gap-x-6 gap-y-2 text-sm text-slate-500">
              <span>✓ Browser-based editing</span><span>✓ Local drafts</span><span>✓ PDF & PNG export</span><span>✓ No watermark</span>
            </div>
          </div>
        </div>
      </section>
      ...
      <div className="cmp-container py-12 sm:py-16">
        <section aria-labelledby="tools-heading">
          <div>
            ...
            <h2 id="tools-heading" className="mt-3 text-3xl font-bold tracking-tight text-slate-900">Start with the job you need to finish.</h2>
            <p className="mt-2 max-w-2xl text-slate-600">Invoice creation is front and center, with the invitation and resume editors one click away.</p>
          </div>
          <div className="mt-8 grid gap-5 lg:grid-cols-3">
            {tools.map((tool) => (
              <Link key={tool.href} to={tool.href} className={`cmp-card group relative ${tool.featured ? 'border-emerald-200 bg-gradient-to-br from-emerald-50 via-white to-teal-50 shadow-md ring-1 ring-emerald-100' : ''}`}>
                {tool.featured && <span className="absolute right-4 top-4 rounded-full bg-emerald-600 px-3 py-1 text-[11px] font-bold uppercase tracking-wide text-white">Featured</span>}
                ...
                <h3 className="mt-5 text-xl font-bold text-slate-900 group-hover:text-indigo-700">{tool.title}</h3>
                <p className="mt-2 text-sm leading-6 text-slate-600">{tool.description}</p>
                <span className="mt-5 inline-flex items-center text-sm font-semibold text-indigo-700">Open tool <span aria-hidden="true" className="ml-1 transition group-hover:translate-x-1">→</span></span>
              </Link>
            ))}
          </div>
        </section>
        ...
        <section className="mt-10 rounded-3xl border border-cyan-100 bg-cyan-50 p-6 sm:p-8" aria-labelledby="pdf-tools-heading">
          <div className="flex flex-col gap-5 sm:flex-row sm:items-center sm:justify-between">
            <div>
              <span className="cmp-eyebrow">More document tools</span>
              <h2 id="pdf-tools-heading" className="mt-3 text-2xl font-bold text-slate-900">Need to edit an existing PDF?</h2>
              <p className="mt-2 max-w-2xl text-sm leading-6 text-slate-600">Use FreePDF Tools for browser-based PDF merging, splitting, rotation, watermarking and conversion without uploading your documents.</p>
            </div>
            <a href="https://freepdfconverter-all-in-one.pages.dev/" className="cmp-secondary-btn shrink-0">Open FreePDF Tools →</a>
          </div>
        </section>
        ...
        <section className="mt-16 grid gap-6 lg:grid-cols-[1.15fr_.85fr]">
          <div className="rounded-3xl bg-slate-900 p-8 text-white shadow-xl sm:p-10">
            <span className="inline-flex rounded-full bg-white/10 px-3 py-1 text-xs font-semibold text-cyan-200">Why CraftMyPage</span>
            <h2 className="mt-4 text-3xl font-bold tracking-tight">Your documents stay in your browser.</h2>
            <p className="mt-4 max-w-2xl leading-7 text-slate-300">The core editors are designed around local processing. Your invitation text, resume details and invoice data do not need to be uploaded to a conversion server just to create the final file.</p>
            <div className="mt-8 grid gap-4 sm:grid-cols-3">
              {[['Local processing', 'Edit and export in the browser you already have open.'], ['Local drafts', 'Save projects in your browser and reopen them later.'], ['Simple exports', 'Download PDF or image files when your design is ready.']].map(([title, body]) => (
                <div key={title} className="rounded-2xl border border-white/10 bg-white/5 p-4"><p className="font-semibold">{title}</p><p className="mt-1 text-sm leading-5 text-slate-400">{body}</p></div>
              ))}
            </div>
            <Link to="/privacy" className="mt-7 inline-flex text-sm font-semibold text-cyan-300 hover:text-white">Read the privacy policy →</Link>
          </div>
          <div className="cmp-surface p-8 sm:p-10">
            ...
            <p className="mt-2 text-sm leading-6 text-slate-600">Browse focused invitation categories before you open the editor.</p>
            <div className="mt-6 grid grid-cols-1 gap-2 sm:grid-cols-2 lg:grid-cols-1 xl:grid-cols-2">
              {categories.map(([label, to]) => <Link key={to} to={to} className="rounded-xl border border-slate-200 px-4 py-3 text-sm font-medium text-slate-700 transition hover:border-indigo-200 hover:bg-indigo-50 hover:text-indigo-700">{label} <span aria-hidden="true">→</span></Link>)}
            </div>
          </div>
        </section>
