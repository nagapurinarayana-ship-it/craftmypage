...
export default function ResumeBuilderPage() {
  ...
  const refreshSaved = useCallback(() => { getAllResumes().then(setSavedResumes).catch(() => setSavedResumes([])) }, [])
  ...
  const updateData = (data: ResumeData) => { if (project) setProject({ ...project, data, updatedAt: Date.now() }) }
  const handleSave = async () => { if (project) { await saveResume(project); refreshSaved() } }
  const handleDelete = async (id: string) => { await deleteResume(id); refreshSaved() }
  ...
  if (!project) {
    return (
      <div className="cmp-tool-shell">
        <div className="mb-8 max-w-3xl"><span className="cmp-eyebrow">Resume Builder</span><h1 className="cmp-tool-title mt-3">Build a resume that looks polished and stays practical.</h1><p className="cmp-tool-subtitle">Choose a focused template, add your experience and projects, then export a ready-to-share A4 PDF. Your resume data stays in your browser.</p></div>
        <section><div className="mb-5 flex items-end justify-between"><div><h2 className="text-2xl font-bold text-slate-900">Choose your template</h2><p className="mt-1 text-sm text-slate-500">Start with the structure that fits your career stage.</p></div></div><div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-4">{TEMPLATES.map((template) => <button key={template.id} type="button" className="group cmp-card text-left" onClick={() => handleNew(template.id)}><div className="flex h-10 w-10 items-center justify-center rounded-xl bg-slate-100 text-sm font-bold text-slate-700 group-hover:bg-indigo-50 group-hover:text-indigo-700">CV</div><h2 className="mt-4 text-lg font-bold text-slate-900 group-hover:text-indigo-700">{template.name}</h2><p className="mt-2 text-sm leading-6 text-slate-500">{template.description}</p><span className="mt-5 text-sm font-semibold text-indigo-600">Use template →</span></button>)}</div></section>
        {savedResumes.length > 0 && <section className="mt-12" aria-label="Saved resumes"><span className="cmp-eyebrow">Local drafts</span><h2 className="mt-2 text-2xl font-bold text-slate-900">Continue a saved resume</h2><div className="mt-5 space-y-3">{savedResumes.map((saved) => <div key={saved.id} className="cmp-surface flex flex-col gap-3 p-4 sm:flex-row sm:items-center sm:justify-between"><div><p className="font-semibold text-slate-900">{saved.data.contact.fullName || 'Untitled resume'}</p><p className="mt-1 text-xs text-slate-500">{saved.name} · Updated {new Date(saved.updatedAt).toLocaleDateString()}</p></div><div className="flex gap-2"><button type="button" className="cmp-secondary-btn px-4 py-2" onClick={() => handleOpen(saved)}>Open</button><button type="button" className="rounded-xl border border-red-200 bg-red-50 px-4 py-2 text-sm font-semibold text-red-700 hover:bg-red-100" onClick={() => handleDelete(saved.id)}>Delete</button></div></div>)}</div></section>}
      </div>
    )
  }
